...
function JobCard({ report, onResolve }) {
  ...
  return (
    <div className="bg-white border border-slate-200 shadow-sm rounded-2xl overflow-hidden mb-5 transition-shadow hover:shadow-md">
      {/* ── Image Header ─────────────────────────────────────────────────── */}
      {report.image_url ? (
      ...
      <div className="p-4 space-y-4">
        ...
        <div>
          ...
          <p className="text-xs text-slate-500 font-mono bg-slate-50 px-2 py-1 rounded inline-block border border-slate-100">
        ...
        {/* Error banner */}
        {error && (
          <div className="p-3 text-xs text-alert bg-alert-light border border-alert/20 rounded-lg">
            {error}
          </div>
        )}
        ...
        <div className="pt-2 border-t border-slate-100 flex flex-col sm:flex-row gap-3">
          <a
            ...
            className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-slate-100 hover:bg-slate-200 border border-slate-200 px-4 py-3.5 text-sm font-semibold text-slate-700 transition focus:outline-none focus:ring-2 focus:ring-slate-300 active:bg-slate-300"
          >
          ...
          <button
            ...
            disabled={resolving}
            className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-primary hover:bg-primary-hover px-4 py-3.5 text-sm font-semibold text-white shadow-md shadow-primary/20 transition focus:outline-none focus:ring-2 focus:ring-primary/40 active:bg-green-700 disabled:opacity-70 disabled:cursor-not-allowed"
          >
            {resolving ? (
              <>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <header className="sticky top-0 z-50 bg-white border-b border-slate-200 shadow-sm px-4 py-3">
        <div className="max-w-xl mx-auto flex items-center justify-between">
          <div className="flex flex-col">
            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Active Route</span>
            <h1 className="text-lg font-extrabold text-slate-800 leading-tight">
              {profile?.full_name || 'Driver'}
            </h1>
          </div>
          <div className="flex items-center gap-3">
            <span className="inline-flex items-center rounded-full bg-blue-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-blue-800 border border-blue-200">
              Collector
            </span>
            <button
              onClick={handleSignOut}
              className="flex h-9 w-9 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:text-slate-800 hover:bg-slate-200 transition focus:outline-none"
              aria-label="Sign out"
            >
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-5 h-5">
      ...
      <main className="px-4 py-6 max-w-xl mx-auto pb-24">
        ...
        {loading ? (
          ...
          // Error State
          <div className="bg-alert-light border border-alert/20 rounded-2xl p-6 text-center text-alert shadow-sm mt-8">
          ...
          <div className="flex flex-col items-center justify-center text-center mt-16 px-6">
            <div className="flex h-20 w-20 items-center justify-center rounded-full bg-green-50 border-8 border-green-100/50 text-green-500 mb-5">
              <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor" className="w-10 h-10">
                <path strokeLinecap="round" strokeLinejoin="round" d="M4.5 12.75l6 6 9-13.5" />
              </svg>
            </div>
            <h2 className="text-xl font-extrabold text-slate-800 tracking-tight mb-2">Route Complete!</h2>
